...
10. [Обеспечение надежности](#10-обеспечение-надёжности)
...
### Шардирование
Будут шардироваться таблицы: Playlist (10 узлов), PlaylistMusic (30) и FavoriteMusic (30) в зависимости от идентификатора.
### Партиционирование
Таблица ListeningHistory будет партиционироваться по дате создания сущностей (т. е. 30 партиций). Это позволит обеспечить более эффективную и быструю работу с нужными данными, поскольку увеличится вероятность попадания их в память и кэш.
...
| Session 	| 626 млн × (8 + 64 + 1) байт = 43	|
...
Данные по пиковому RPS взяты из пункта 2. Из-за малого числа ДЦ полностью равномерного распределения нагрузки добиться не удалось, поэтому для облегчения расчетов за нагрузку на датацентр будет взята средняя нагрузка на каждый датацентр (т.е. общее RPS ÷ число датацентров).
Для обеспечения надежности в расчетах будем учитывать возможность отключения датацентров и брать требуемую нагрузку в 10 раза больше вычисленной на датацентр.
> Прослушивание музыки осуществляется при помощи CDN, поэтому и числа в вычислениях будут использоваться несколько другие; суть расчетов при этом меняться не будет.

| Сервис 	| Целевой пиковый RPS 	| Нагрузка на сеть, Гбит/с 	|
|---	|---	|---	|
| Authentication 	| 183 / 3 × 10 = 610 	| 0.072 / 3 × 10 = 0.24 	|
| Music listening 	| 161361 / 49 × 5 = 16465 	| 5809 / 49 × 5 = 592.5 	|
| Profile 	| (182 + 3) / 3 × 10 = 617 	| 1.1 / 3 × 10 = 3.7 	|
| Artist 	| 363 / 3 × 10 = 1210 	| 2.2 / 3 × 10 = 7.3 	|
| Playlists 	| (15 + 363) / 3 × 10 = 1260 	| (0.006 + 0.05) / 3 × 10 = 0.2 	|
| Search 	| 727 / 3 × 10 = 2424 	| 26.17 / 3 × 10 = 87.2 	|
| Recommendations 	| (15.7 млн ÷ 86400) / 3 × 10 = 606 	| (15.7 млн * 250 КБ / 86400 * 3) / 3 × 10 = 3.5 	|

Приведем соотношения между характером бизнес-логики и требуемыми ресурсами:
- **легкая бизнес-логика**: 1 CPU на 100 RPS и 1 ГБ RAM на 1000 RPS;
- **средняя бизнес-логика**: 1 CPU на 50 RPS и 1 ГБ RAM на 500 RPS;
- **тяжелая бизнес-логика**: 1 CPU на 25 RPS и 1 ГБ RAM на 250 RPS;
- **Nginx** может поддерживать до 10000 одновременных HTTPS-соединений в секунду на 24 ядрах CPU[^12].

Используя приведенные данные, вычислим необходимые аппаратные возможности для каждого сервиса.
| Сервис 	| RPS 	| Характер бизнес-логики 	| CPU 	| RAM, ГБ 	|
|---	|---	|---	|---	|---	|
| Music listening 	| 16465 	| средняя 	| 330 	| 33 	|
| Authentication 	| 610 	| легкая 	| 7 	| 1 	|
| Profile 	| 617 	| средняя 	| 13 	| 2 	|
| Artist 	| 1210 	| легкая 	| 13 	| 2 	|
| Playlists 	| 1260 	| средняя 	| 26 | 3 	|
| Search 	| 24243 	| тяжелая 	| 40 	| 4 	|
| Recommendations 	| 606 	| тяжелая 	| 25 	| 3	|
| API-Gateway 	| 45011 	| легкая 	| 451   | 46    |
| Nginx 	| 45011 	| SSL-handshake 	| 24 * 5   | 80    |

**Итого**: на 1 ДЦ с развернутыми в Kubernetes сервисами нужно выделять 1025 ядер CPU и 174 ГБ RAM.

Проведем этот же расчет для баз данных. Будем считать, что на 100 RPS требуется 1 ядро процессора, а RAM найдем исходя из требуемого размера индекса (в расчетах уже учтены число узлов при шардировании и партиционировании, а также дополнительные реплики для отказоустойчивости и уменьшения нагрузки на чтение)
| Сервис 	| RPS 	| CPU 	| RAM, ГБ 	|
|---	|---	|---	|---	|
| Cassandra 	| 65258 	| 66 	| 354 / 3 = 118 	|
| Redis 	| 183 	| 2 	| 42 / 3  = 14 	|
### Конфигурация серверов
Конфигурации серверов были собраны с использованием конструктора Broadberry[^13].  
Для проверки стоимости аренды был использован калькулятор Amazon AWS[^14].
| Сервис 	| Конфигурация 	| Ядра 	| На 1 ДЦ 	| Покупка 1 	| Амортизация 	| Аренда 1 	|
|---	|---	|---	|---	|---	|---	|---	|
| Kubernode 	| 2x6554S/2x16Gb/1xNVMe512Gb/2x25Gb/s 	| 72 	| 15 	| $11260.6 	| $188 	| $37551.5 	|
| Cassandra + Redis 	| 6326/3x32Gb/4xNVMe30Tb/2x25Gb/s 	| 16 	| 7 	| $28395.5 	| $473.3 	| $2150.6 	|
...
[^12]: [Nginx performance testing](https://blog.nginx.org/blog/testing-the-performance-of-nginx-and-nginx-plus-web-servers)
[^13]: [Broadberry servers](https://www.broadberry.eu/rackmount-servers)
