...
Для проведения расчета был принят _ряд допущений_:
...
2. Kafka Streams преобразует данные о действии в удобный для работы JSON и сериализует его.
3. Собираем действия по пользователям в течение 1-2 минут.
4. Массивы с уже имеющимися данными пользовательских действий считываются из Cassandra и обновляются с учетом пришедших данных.
5. Записываем обновленные пользовательские действия в таблицу Cassandra.
  
> [!NOTE]
> Для обеспечения высокой производительности были приняты _следующие решения_:
> * запись в Cassandra совершается с некоторой задержкой (1-2 минуты, как описано в п.3 алгоритма), чтобы не так сильно нагружать базу данных частым обновлением данных;
> * по этой же причине вставлять данные нужно батчами, сразу для нескольких пользователей;
> * массив с сохраненными данными пользователей необходимо считывать и обновлять в Kafka - так удастся реализовать логику ограничения числа элементов в списках.

### Content-Based фильтрация
Этот алгоритм предназначен для формирования плейлистов с рекомендациями на основе пользовательских предпочтений и действий.

**Шаги алгоритма:**
1. Читаем данные из таблиц RecentActivity и FavoriteMusic по данному пользователю (по user_id).
2. По случайной выборке из нескольких треков пользователя анализируем его предпочтения с точки зрения метаданных трека (например, жанра или исполнителя); сделать это можно простым подсчетом ниболее часто встречающихся элементов.
3. 
