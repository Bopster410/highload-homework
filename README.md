...
> _Примечание:_  
> Для проведения более тщательного анализа будем сохранять историю пользователей за более длительный срок (например, за несколько лет)
...
| PlaylistMusic 	| 8 млрд × 50 × (8 + 8 + 1 + 64 + 64 + 255 + 2) байт = 149756.7	|
| FavoriteMusic 	| 626 млн × 500 × (8 + 8 + 1 + 64 + 64 + 255 + 2) байт = 117184.6	|
...
| ListeningHistory 	| 626 млн × 30 × 40 × (8 + 8 + 8 + 2 + 64 + 64 + 255 + 2 + 1 + 1) байт = 288938.7 |
| RecentActivity 	| 7 млн × (8 + 20 × 200 × 4) байт = 9332.8    |

Для проведения расчета был принят ряд допущений:
* среднее число альбомов на одного исполнителя равно 5
* среднее число песен в плейлисте равное 50;
* среднее число песен в избранном равно 500;
...
* средний размер сериализованного JSON-а с информацией о песне равен 200 байт, сохранять будем последние 20 действий.

## 7. Алгоритмы
| Алгоритм 	| Область применения 	| Мотивация 	|
|:---:	|:---:	|:---:	|
| Сохранение пользовательских событий 	| Система рекомендаций 	| Это позволит учитывать поведение пользователя при составлении рекомендаций	|
| Потоковая обработка данных<br>(Kafka Streams) 	| Создание реплик БД в ClickHouse и ElasticSearch 	| Это позволит реплицировать данные в реальном времени и поддерживать наиболее актуальную их версию 	|
...
1. Данные о пользовательских действиях попадают в соответствующие топики в Kafka.
2. Оттуда они считываются и попадают в Kafka Streams.
3. Kafka Streams преобразует данные в удобный для работы JSON и сериализует его.
4. Из Kafka Streams полученная строка добавляется в соответствующий список таблицы RecentActivity в Cassandra.
5. Если длина списка слишком велика (более 20), то удалить самый старый элемент (первый в списке).

### Создание реплик БД в ClickHouse
1. Необходимые данные из Cassandra и события, связанные с действиями пользователей, передаются в Kafka.
2. Они объединяются и преобразуются для удобства дальнейшего анализа.
3. Обработанные данные считываются и добавляются в ClickHouse.

В данном алгоритме использовать Kafka нужно в первую очередь для потоковой обработки больших объемов данных и их объединения с пользовательскими событиями. Это позволит в дальнейшем производить более удобную аналитику.
### Создание реплик БД в ElasticSearch
1. Необходимые данные из Cassandra передаются в Kafka.
2. Они преобразуются для удобства дальнейшей работы.
3. Обработанные данные считываются и добавляются в ElasticSearch.

В данном случае использование потоковой обработки информации обусловлено в первую очередь необходимостью поддерживать данные в ElasticSearch в максимально актуальном состоянии и возможностью быстро и эффективно передавать большие объемы информации.
## 8. Технологии
